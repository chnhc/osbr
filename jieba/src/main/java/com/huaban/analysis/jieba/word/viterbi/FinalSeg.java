package com.huaban.analysis.jieba.word.viterbi;


import com.huaban.analysis.jieba.word.CharacterUtil;
import com.huaban.analysis.jieba.word.Pair;
...
public class FinalSeg {
    ...
    public FinalSeg(WordDictionary wordDict) {
    ...
    public void viterbi(String sentence, List<Word> tokens) {
        ...
        for (int i = 0; i < sentence.length(); ++i) {
            ...
            if (pos == 'B')
                ...
                tokens.add(Word.createWord(sentence.substring(begin, i + 1),wordDict));
                next = i + 1;
            } else if (pos == 'S') {
                tokens.add(Word.createWord(sentence.substring(i, i + 1),wordDict));
                next = i + 1;
            }
        }
        if (next < sentence.length()) tokens.add(Word.createWord(sentence.substring(next),wordDict));
    }
    ...
    private void processOtherUnknownWords(String other, List<Word> tokens) {
        ...
        while (mat.find()) {
            if (mat.start() > offset) {
                tokens.add(Word.createWord(other.substring(offset, mat.start()),wordDict));
            }
            tokens.add(Word.createWord(mat.group(),wordDict));
            offset = mat.end();
        }
        if (offset < other.length()) tokens.add(Word.createWord(other.substring(offset),wordDict));
    }
}
