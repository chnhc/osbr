...
import com.huaban.analysis.jieba.word.viterbi.FinalSeg;
...
public class JiebaSegmenter {
    ...
    public JiebaSegmenter(WordDictionary wordDict) {
        this.wordDict = wordDict;
        this.finalSeg = new FinalSeg(wordDict);
    }
        ...
        for (int i = 0; i < paragraph.length(); ++i) {
            ...
            if (CharacterUtil.ccFind(ch))
                ...
                if (wordDict.containsWord(paragraph.substring(i, i + 1)))
                    ...
                    tokens.add(new SegToken(Word.createWord(paragraph.substring(i, i + 1),wordDict), offset, ++offset));
            }
        }
        if (sb.length() > 0)
            if (mode == SegMode.SEARCH) {
    ...
    public List<Word> sentenceProcess(String sentence) {
        ...
        while (x < N) {
            ...
            if (y - x == 1)
                ...
                if (buf.length() > 0) {
                    if (buf.length() == 1) {
                        tokens.add(Word.createWord(buf,wordDict));
                        buf = "";
                    } else {
                ...
                tokens.add(Word.createWord(lWord,wordDict));
            }
            x = y;
        }
        if (buf.length() > 0) {
