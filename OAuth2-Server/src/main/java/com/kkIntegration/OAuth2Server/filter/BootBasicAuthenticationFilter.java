package com.kkIntegration.OAuth2Server.filter;
...
public class BootBasicAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (this.isHasClientDetails(request) == null) {
            //BaseResponse bs = HttpResponse.baseResponse(HttpStatus.UNAUTHORIZED.value(), "请求中未包含客户端信息");
            HttpWriterUtils.writerResponse(response , BaseResponseStatus.BasicAuth_Failed);
            return;
        }
