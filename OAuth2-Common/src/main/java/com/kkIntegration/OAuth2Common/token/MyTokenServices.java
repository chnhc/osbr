package com.kkIntegration.OAuth2Common.token;
...
public class MyTokenServices extends DefaultTokenServices {
    ...
    private int accessTokenValiditySeconds = 60 * 60 * 12 ; // default 12 hours.60 * 60 * 12
    ...
    public MyTokenServices(String bearerType) {
    ...
    public OAuth2Authentication loadAuthentication(String accessTokenValue) throws AuthenticationException,
        ...
        if (accessToken == null) {
           ...
           String clientId = authentication.getOAuth2Request().getClientId();
            ...
            // 之前
            //MyOAuth2AccessToken myOAuth2AccessToken = (MyOAuth2AccessToken) accessToken;
            // 自动续签成功
            //myOAuth2AccessToken.autoRefresh();
            // 以及下方的 result.setDetails(accessToken);注解
            //
            // 自动续签成功 改成以下
            // 为当前线程 工具 设置新的token ，后添加
            NewTokenCurrentThreadUtils.setNewToken(accessToken.getValue());
        }

        OAuth2Authentication result = tokenStore.readAuthentication(accessToken);
        Authentication authentication = result.getUserAuthentication();

        new OAuth2Authentication(result.getOAuth2Request(),authentication);
    ...
    private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
        MyOAuth2AccessToken token = new MyOAuth2AccessToken(UUID.randomUUID().toString(), bearerType);
